...
const moveInterval = 100; // 蛇每次移动的时间间隔（单位：毫秒）
const moveSpeed = 0.1; // 蛇的平滑移动速度（相对格子大小）
...
function gameLoop(currentTime) {
    ...
    // 计算蛇头的平滑移动
    let head = { x: snake[0].x + direction.x * moveSpeed, y: snake[0].y + direction.y * moveSpeed };

    // 如果蛇头已经完全跨过一个格子，才将其固定到一个新的格子
    if (Math.abs(head.x - Math.floor(head.x)) < moveSpeed && Math.abs(head.y - Math.floor(head.y)) < moveSpeed) {
        head = { x: Math.floor(head.x), y: Math.floor(head.y) };
    }
