...
const moveSpeed = 10; // 蛇的移动速度（格子/秒）

function moveSnake(currentTime) {
    const deltaTime = (currentTime - lastUpdateTime) / 1000; // 计算时间差（秒）
    lastUpdateTime = currentTime;

    // 更新蛇的位置
    const distance = moveSpeed * deltaTime; // 计算本次移动的距离
    if (distance >= 1) {
...
requestAnimationFrame(moveSnake); // 启动蛇的移动逻辑
requestAnimationFrame(renderGame); /
