...
// 默认速度
let moveInterval = 100; // 默认移动间隔时间（毫秒）
...
setSpeedButton.addEventListener("click", () => {
    speedInput.style.display = "inline-block"; // 显示输入框
    speedHint.style.display = "block"; // 显示提示
});

// 监听速度输入框变化
speedInput.addEventListener("input", event => {
    const newSpeed = parseInt(event.target.value); // 获取输入的值
    if (newSpeed >= 50 && newSpeed <= 500) { // 限制速度范围
        moveInterval = newSpeed; // 更新移动间隔时间
    } else {
        alert("请输入 50 到 500 之间的数字！");
    }
});
